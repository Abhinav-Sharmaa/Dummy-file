...
final class TestCaseGenerator
{
    ...
    public static function generate(string $ticket, string $url): array
    {
        ...
        $cases[] = self::make('iframe', 'Banking widget iframe: locate, authenticate, validate', [
            'Target URL is reachable',
            "Page on {$host} embeds the third-party banking widget in an iframe",
            'Widget test credentials are available (sample target: qa_demo_user / S3cure!Demo)',
        ], [
            "Navigate to {$url}",
            'Locate the banking widget iframe on the page',
            'Switch context into the iframe via Playwright frame locator',
            'Submit the widget form empty and verify an inline validation error inside the frame',
            'Fill the username field with the test credential',
            'Fill the password field with the test credential',
            'Submit the widget login form',
            'Verify the widget shows an authenticated/success state inside the frame',
        ], 'Iframe is present and reachable through frameLocator; empty submit is rejected with an in-frame error; valid credentials produce an in-frame success state. Applies on all three viewports.');
