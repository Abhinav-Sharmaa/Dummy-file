...
function main() {
  ...
  if (dry) {
    const list = IS_WIN
      ? spawnSync("npx playwright test --list", { cwd: RUNNER, env, encoding: "utf8", shell: true })
      : spawnSync("npx", ["playwright", "test", "--list"], { cwd: RUNNER, env, encoding: "utf8" });
  ...
  const pw = IS_WIN
    ? spawnSync("npx playwright test", { cwd: RUNNER, env, stdio: "inherit", timeout: 15 * 60 * 1000, shell: true })
    : spawnSync("npx", ["playwright", "test"], { cwd: RUNNER, env, stdio: "inherit", timeout: 15 * 60 * 1000 });
