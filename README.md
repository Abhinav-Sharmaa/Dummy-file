...
process.stdout.write(’  - ’ + v.name + ’ (’ + v.width + ‘x’ + v.height + ’)… ’);
...
const client = opts.client, project = opts.project, url = opts.url;
const change = opts.change, ticket = opts.ticket, screenshots = opts.screenshots;
const date = new Date().toLocaleString();

let sections = ‘’;
for (const name in screenshots) {
const d = screenshots[name];
sections += ‘<section class="vp">’
+ ‘<h2>’ + name + ’ <span class="size">’ + d.viewport.width + ’ x ’ + d.viewport.height + ‘</span></h2>’
+ ‘<img src="data:image/png;base64,' + d.base64 + '" alt="' + name + ' screenshot" />’
+ ‘</section>’;
}
...
+ ‘.change-box { background: #f9fafb; border-left: 4px solid #2563eb; padding: 14px 18px; margin-top: 20px; font-size: 13px; }’
...
+ ‘</table>’
+ ‘<div class="change-box"><h3>Change Description</h3><div>’ + escapeHtml(change || ‘No description provided.’) + ‘</div></div>’
+ ‘</div>’
+ sections
+ ‘<div class="signoff"><h2>Sign-off</h2>’
+ ‘<div class="sigrow">’
...
if (!args.url) {
console.log(‘UAT Generator - responsive PDF reports’);
console.log(’’);
console.log(‘Usage:’);
console.log(’  node generate-uat.js –url <url> [options]’);
console.log(’’);
console.log(‘Options:’);
console.log(’  –url       URL to test (required)’);
console.log(’  –client    Client name’);
console.log(’  –project   Project name’);
console.log(’  –change    Change description’);
console.log(’  –ticket    Ticket / reference number’);
console.log(’  –output    Output PDF filename (default: uat-<timestamp>.pdf)’);
process.exit(1);
}
...
change: args.change, ticket: args.ticket, screenshots: screenshots,
});
